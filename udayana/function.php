<?php
$koneksi = mysqli_connect('localhost', 'root', '', 'udayana');
...
function registerAdmin()
{
    ...
    $alamat = htmlspecialchars($_POST['alamat']);
    $tgl_lahir = $_POST['tgl_lahir'];
    $query_user = "INSERT INTO admin VALUES ('', '0', '$nip','$nama', '$email', '$alamat', '$tgl_lahir','$password')";
    $exe = mysqli_query($koneksi, $query_user);
...
function login()
{
    ...
    $query3 = mysqli_query($koneksi, "SELECT * FROM admin where nip = '$username'");
...
function editAdmin($id_admin)
{
    ...
    $email = htmlspecialchars($_POST['email']);
    $tgl_lahir = htmlspecialchars($_POST['tgl_lahir']);
    $queryProfil = "UPDATE admin SET nama = '$nama', nip = '$nip', email = '$email', alamat = '$alamat', tanggal_lahir = '$tgl_lahir' WHERE id_admin = '$id_admin'";
    $exe = mysqli_query($koneksi, $queryProfil);
    if (!$exe) {
        die('Error pada database');
    }
